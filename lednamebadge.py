...
try:
    from badge_common import BadgeTextParser, BITMAP_NAMED, FONT_11X44, CHAR_OFFSETS
    _HAVE_BADGE_COMMON = True
except ImportError:
    _HAVE_BADGE_COMMON = False
...
class SimpleTextAndIcons:
    """
    Wrapper around BadgeTextParser from badge_common for backward compatibility.
    Provides the same API as the original SimpleTextAndIcons class.
    """

    def __init__(self):
        if _HAVE_BADGE_COMMON:
            self._parser = BadgeTextParser()
            # Expose shared data for backward compatibility
            self.font_11x44 = FONT_11X44
            self.bitmap_named = BITMAP_NAMED
            self.bitmap_builtin = {}
            for name in BITMAP_NAMED:
                self.bitmap_builtin[BITMAP_NAMED[name][2]] = BITMAP_NAMED[name]
    ...
    @staticmethod
    def _pack_icon_ascii_static(ascii_art: str, ncols: int, ctrl: str):
        """
    ...
    def add_preload_img(self, filename):
        ...
        if _HAVE_BADGE_COMMON:
            self._parser.add_preload_img(filename)
        else:
    ...
    def are_preloaded_unused(self):
        """Still used by main, but deprecated. Please use ":"-notation for bitmap() / bitmap_text()"""
        if _HAVE_BADGE_COMMON:
            return self._parser.are_preloaded_unused()
    ...
    def bitmap_char(self, ch):
        ...
        """
        if _HAVE_BADGE_COMMON:
            return self._parser.bitmap_char(ch)

        # Fallback to original implementation
        if ord(ch) < 32:
            if ch in self.bitmap_builtin:
                return self.bitmap_builtin[ch][:2]
            self.bitmaps_preloaded_unused = False
            return self.bitmap_preloaded[ord(ch)]
        o = self.char_offsets[ch]
        return self.font_11x44[o:o + 11], 1
    ...
    def bitmap_text(self, text):
        ...
        """
        if _HAVE_BADGE_COMMON:
            return self._parser.parse_text(text)
        ...
        def replace_symbolic(m):
            ...
            return self.bitmap_named[name][2]
    ...
    def bitmap_img(file):
        ...
        """
        if _HAVE_BADGE_COMMON:
            return BadgeTextParser.bitmap_img(file)
    ...
    def bitmap(self, arg):
        ...
        """
        if _HAVE_BADGE_COMMON:
            return self._parser.bitmap(arg)
        if os.path.exists(arg):
            return self.bitmap_img(arg)
        return self.bitmap_text(arg)


# Initialize "git" icon after class is defined
SimpleTextAndIcons.bitmap_named["git"] = SimpleTextAndIcons._pack_icon_ascii_static(
    """
         //
          //
       //  //
...
# Update bitmap_builtin with the new icon
SimpleTextAndIcons.bitmap_builtin[SimpleTextAndIcons.bitmap_named["git"][2]] = SimpleTextAndIcons.bitmap_named["git"]
